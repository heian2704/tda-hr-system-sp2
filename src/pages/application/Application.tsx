      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        {showStatusEditAlert && (
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[2000] bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg">
            {translations?.employeePage?.statusUpdate ||
              "Status updated successfully."}
          </div>
        )}
        {showCreateAlert && (
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[2000] bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg">
            {translations?.applicationPage?.createdSuccessfully ||
              "Application created successfully."}
          </div>
        )}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-4">
          <h2 className="text-xl font-semibold text-gray-900">
            {translations?.applicationPage?.title || "Job Applications"}
          </h2>
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative w-full md:w-80">
              <input
                ...
                placeholder={
                  translations?.applicationPage?.searchPlaceholder ||
            ...
            {/* <BulkStatusUpdateBar
              ...
              currentStatusById={Object.fromEntries(
                pageSlice.map((a) => [a._id, a.status || ""])
              )}
              onApplied={(ns: string) => {
                refetch();
                setSelected(new Set());
            ...
            >
              ...
              {translations?.applicationPage?.createButton ||
                "Create Application"}
            </button> */}
          </div>
        </div>
              ...
              <tr className="text-left text-gray-600 border-b border-gray-200 bg-gray-50">
                <th className="py-3 px-4 w-10">
                  <input
                    ...
                    checked={
                      pageSlice.length > 0 &&
                      pageSlice.every((r) => selected.has(r._id))
                    }
                    onChange={(e) => {
                      const next = new Set(selected);
                      if (e.target.checked)
                        pageSlice.forEach((r) => next.add(r._id));
                ...
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.nameColumn || "Name"}
                </th>
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.phoneNumberColumn || "Phone"}
                </th>
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.positionColumn || "Position"}
                </th>
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.applicationStatusColumn ||
                    "Application Status"}
                </th>
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.dateColumn || "Date"}
                </th>
                <th className="py-3 px-4 font-semibold">
                  {translations?.applicationPage?.addressColumn || "Address"}
                </th>
              </tr>
            </thead>
            <tbody>
              {pageSlice.length === 0 ? (
                <tr>
                  ...
                  >
                    {query
                      ? translations?.applicationPage?.noSearchResults ||
                        "No applications match your search."
                      : translations?.applicationPage?.noData ||
                    ...
                    <td className="py-3 px-4 text-gray-700 max-w-md">
                      {a.address && a.address.trim().length > 0 ? (
                        <button
                          ...
                          title={`${
                            translations?.applicationPage?.viewOnMap ||
                            "View on map:"
                          } ${a.address}`}
                        >
                          <MapPin className="w-4 h-4" />
                          <span className="truncate max-w-[18rem] text-left">
                            {a.address}
                          </span>
                        </button>
                      ) : (
        ...
        <Dialog open={mapOpen} onOpenChange={setMapOpen}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>
                {translations?.applicationPage?.mapTitle || "Location"}
              </DialogTitle>
            </DialogHeader>
