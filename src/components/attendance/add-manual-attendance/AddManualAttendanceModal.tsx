...
export const AddManualAttendanceModal: React.FC<Props> = ({ employeeId, employeeName, onCreated, triggerClassName }) => {
  ...
  const combineToIso = (d: string, t: string) => {
    // Expect d as YYYY-MM-DD and t as HH:mm
    if (!d || !t) return "";
  ...
  const onSubmit = async () => {
    ...
    console.log("Submitting manual attendance", payload);
    ...
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetForm(); }}>
      <DialogTrigger asChild>
        <button
          ...
          className={
            triggerClassName ||
            "inline-flex items-center justify-center h-7 min-w-[110px] px-3 rounded-full text-xs font-medium whitespace-nowrap bg-white border border-gray-300 text-gray-700 hover:bg-gray-50"
          }
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add Attendance With Back Date{employeeName ? ` — ${employeeName}` : ""}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={onSubmit} disabled={loading || !tokenReq}>
...
export default AddManualAttendanceModal;
